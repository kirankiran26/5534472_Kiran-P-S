...
int* dynamicArray(int n, int queries_rows, int queries_columns, int** queries, int* result_count) {
    ...
    int *seqCapacity = calloc(n, sizeof(int)); 
    ...
    for (int i = 0; i < queries_rows; i++) {
        ...
        if (type == 1) {
            ...
            if (seqCapacity[idx] == 0) {
                seqCapacity[idx] = 2;
                seqList[idx] = malloc(seqCapacity[idx] * sizeof(int));
            } else if (seqSizes[idx] >= seqCapacity[idx]) {
                seqCapacity[idx] *= 2;
                seqList[idx] = realloc(seqList[idx], seqCapacity[idx] * sizeof(int));
            }
